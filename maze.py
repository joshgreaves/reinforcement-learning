...
def train_ppo():
    ...
    intrinsic_net = nets.ConvNetwork128(128, 128, input_channels=4)

    ppo = rl.PPO(factory, policy, value, embedding_network=conv, device=torch.device('cuda'), gamma=0.999,
                 experiment_name='holodeck_maze_explore', gif_epochs=5, epsilon=0.4, intrinsic_network=intrinsic_net)
    ppo.train(100, rollouts_per_epoch=18, max_episode_length=300, environment_threads=9, data_loader_threads=10)
