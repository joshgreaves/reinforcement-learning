...
class DoomEnvironment(RLEnvironment):
    def __init__(self, prev_frames=4, skiprate=1):
        ...
        # self._game.set_doom_scenario_path("scenarios/basic.wad")
        ...
        # self._game.set_living_reward(-0.1)
    ...
    def step(self, action):
        ...
        if t:
            return self._imgs, np.array((-1.0)), t

        state = self._game.get_state()
        screen_buf = state.screen_buffer
        self._process_state(screen_buf)

        return self._imgs, np.array((r)), t
...
class PolicyNetwork(nn.Module):
    def __init__(self, action_dim=3, hidden_dim=100):
...
def main():
    ...
    ppo(factory, policy, value, multinomial_likelihood, embedding_net=conv, epochs=1000, rollouts_per_epoch=50,
        max_episode_length=3000, gamma=0.999, policy_epochs=5, batch_size=256, epsilon=0.2, environment_threads=10,
        device=torch.device('cuda'), lr=1e-3, betas=(0.1, 0.5), weight_decay=0.01, gif_epochs=1)
